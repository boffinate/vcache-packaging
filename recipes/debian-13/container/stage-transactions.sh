...
#   CACHETAG_VERSION    the baseline cachetag Debian version
...
: "${BASE_VERSION:?}" "${BASE_ABI:?}" "${CACHETAG_VERSION:?}"
...
cp "/out/libvmod-cachetag_${CACHETAG_VERSION}_${DEB_HOST_ARCH}.deb" /repo/
...
###############################################################################
if [ "$WITH_DEV" = 1 ]; then
	apt-get install -y vinyl-cache vinyl-cache-dev libvmod-cachetag
else
	apt-get install -y vinyl-cache libvmod-cachetag
fi
baseline_rc=$?
...
cachetag_before=$(inst libvmod-cachetag)
echo "installed baseline: vinyl-cache=$vinyl_before vinyl-cache-dev=$dev_before libvmod-cachetag=$cachetag_before"
[ "$vinyl_before" = "$BASE_VERSION" ] || { echo "E: baseline vinyl-cache version wrong"; exit 1; }
[ "$cachetag_before" = "$CACHETAG_VERSION" ] || { echo "E: baseline cachetag version wrong"; exit 1; }

cat > /tmp/probe.vcl <<'VCL'
vcl 4.1;
...
echo "baseline VCL compile with 'import cachetag': $vcl_before"
...
cachetag_after=$(inst libvmod-cachetag)
echo "vinyl-cache:        $vinyl_before -> $vinyl_after"
echo "vinyl-cache-dev:    $dev_before -> $dev_after"
echo "libvmod-cachetag:   $cachetag_before -> $cachetag_after"
echo "--- dpkg -l for the cohort ---"
dpkg -l vinyl-cache vinyl-cache-dev libvmod-cachetag 2>/dev/null | tail -n +6
...
if [ -x /usr/sbin/vinyld ]; then
	...
	echo "--- vinyld -C -f probe.vcl (import cachetag) ---"
	head -n 20 /tmp/vcl-after.out
else
	vcl_after=no-vinyld
fi
echo "post-transaction VCL compile with 'import cachetag': $vcl_after"
...

{
	...
	printf 'RESULT cachetag=%s->%s\n' "$cachetag_before" "$cachetag_after"
