...
def _grid_html(grid: dict, axis_label: str = "VMOD \\ engine") -> str:
    counts = grid["counts"]
    labels = grid.get("column_labels", {})
    head_cols = "".join(f'<th class="col">{_esc(labels.get(col, col))}</th>' for col in grid["columns"])
    body_rows = []
  ...
  <div class="matrix-scroll">
    <table class="matrix">
      <thead><tr><th class="corner">{_esc(axis_label)}</th>{head_cols}</tr></thead>
...
def render_html(grids: list, generated_at: str,
                page_context: str = "Vinyl Cache and Varnish Cache",
                matrix_key: str = "Rows are modules, columns are engine versions. Green: works. Red: doesn't — usually upstream doesn't support that engine yet. Amber edge: API names translated between Vinyl and Varnish. Grey: not tested.",
                matrix_note: str = "Trunk columns are source-build and load checks, not packages. Packages, where provided, are built only from pinned release engines.",
                axis_label: str = "VMOD \\ engine",
                normalization_legend: str = SOURCE_NORMALIZATION_LEGEND,
                normalization_help: str = SOURCE_NORMALIZATION_HELP) -> str:
...
<header class="page">
  ...
  <div class="legend">
    ...
    <span class="legend-help" title="{_esc(normalization_help)}"><i class="swatch sw-translation"></i>{_esc(normalization_legend)}</span>
...
<main>
  <p class="matrix-key">{_esc(matrix_key)}</p>
  <p class="matrix-note">{_esc(matrix_note)}</p>
  <div class="target-matrices">
    {"".join(_grid_html(grid, axis_label) for grid in grids)}
  </div>
</main>
