"""Strict parser for the restricted YAML subset used by cachetag release manifests.

The registry manifests are YAML-shaped (see registry/README.md) but this parser
deliberately accepts only a very small subset:
...
Everything else - anchors, aliases, tags, flow mappings, non-empty flow
sequences, multi-line scalars, multiple documents, tabs, merge keys, comments
that trail a value - is a hard error. There is no type coercion: every scalar is
returned as a string and the schema layer in ``manifest.py`` applies typing.
That avoids YAML's implicit-typing surprises (``23.0`` becoming a float,
``no`` becoming ``False``) in files whose entire purpose is exact identity.
...
def parse_file(path) -> dict:
    """Parse a manifest file. Raises ManifestSyntaxError or OSError."""
    path = Path(path)
...
def parse(text: str, path: str = "<string>") -> dict:
    ...
    if not lines:
        raise ManifestSyntaxError(path, 1, "manifest is empty")
    if lines[0].indent != 0:
    ...
    if not isinstance(value, dict):
        raise ManifestSyntaxError(path, lines[0].lineno, "top level of a manifest must be a mapping")
    return value
...
def _tokenize(text: str, path: str) -> list:
    ...
    for lineno, raw in enumerate(text.split("\n"), start=1):
        if "\r" in raw:
            raise ManifestSyntaxError(path, lineno, "carriage return found; manifests must use LF line endings")
        if "\t" in raw:
        ...
        if stripped.startswith("---") or stripped.startswith("..."):
            raise ManifestSyntaxError(path, lineno, "document markers are not supported; one manifest per file")
        if indent % 2 != 0:
        ...
        if stripped.startswith("-") and not stripped.startswith("- "):
            # A plain scalar may not start with '-' followed by non-space at the
            # start of a line, because it is indistinguishable from a sequence
            # entry to a human reader.
...
def _parse_mapping(lines: list, index: int, indent: int, path: str):
    ...
    while index < len(lines) and lines[index].indent == indent:
        ...
        if not KEY_RE.match(key):
            raise ManifestSyntaxError(path, line.lineno, f"invalid key {key!r}; use lower_snake_case")
        if key in result:
