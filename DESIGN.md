...
- `vmod-batch.yml` — local reusable workflow: downloads one homogeneous batch's engine and exact source artifacts, then delegates to `tools/vmod_batch.py`. Cells run sequentially in separate work directories and disposable containers; the job uploads one result artifact and, when `release.yml` requests them, one package artifact containing cell-addressable directories.
