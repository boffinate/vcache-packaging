...
Rules: each top-level target is the complete execution contract: image, package format, native standard and CPU-parallel runners, Docker platform, and expected binary-package architecture. The required `runner` field selects GitHub-hosted capacity for serial or I/O-bound work; the required `build_runner` field selects the runner for CPU-parallel work. Matrix expansion chooses between them from the existing job and VMOD build metadata, so workflow callers provide no workload-class input. Engine and Cargo VMOD rows use `build_runner`; Autotools VMOD and package-cohort rows use `runner`. The measured hybrid keeps x64 CPU-parallel work on Blacksmith's 4-vCPU runner and everything else, including all ARM work, on GitHub-hosted native runners. `TARGET_RUNNER` exposes `runner` for fixed target-specific workflows. Engine rows only select their target ids. `kind: release` requires `tarball_url` + `sha256`; `kind: trunk` requires `git_url` + `branch` and forces `packages: "false"`. Trunk engines carry a self-named `series` (`vinyl-trunk`); the resolution rule never consults `series` for trunk engines. `packages: "true"` requires a release engine and quoted `package_revision` matching `[1-9][0-9]*`; it is valid for either family. The one revision applies to the engine package and every VMOD in that engine's release set. `toolchains.rust`, when Cargo VMODs exist, is the one exact Rust version and bootstrap contract for every target. Compat and package jobs run on every listed target.
...
- `matrix.yml` — workflow_dispatch + weekly: expand release lane → engine jobs and one fetch job per distinct VMOD source → bounded six-cell VMOD batches through `vmod-batch.yml` → merge → render → commit state to `ci-state/matrix` → deploy Pages. Main-branch runs only publish.
...
- `vmod-batch.yml` — local reusable workflow: downloads one homogeneous batch's engine from its native-target bundle and the exact source bundles it needs, then delegates to `tools/vmod_batch.py`. Cells run sequentially in separate work directories and disposable containers; the job uploads one result artifact and, when `release.yml` requests them, one package artifact containing cell-addressable directories.
