...
- `matrix.yml` — workflow_dispatch + weekly: expand release lane → engine jobs and one fetch job per distinct VMOD source → bounded VMOD shards through `vmod-shard.yml` (fail-fast off, never red on cell failure) → merge → render → commit state to `ci-state/matrix` → deploy Pages. Main-branch runs only publish.
...
- `vmod-shard.yml` — local reusable workflow: expands one at-most-128-row VMOD shard, downloads the source artifact selected during expansion, builds each cell, and uploads its result (plus package artifacts when `release.yml` requests them). The parent calls shards serially; it retains the familiar one-cell-per-job UI without exceeding GitHub's 256-job matrix limit.
