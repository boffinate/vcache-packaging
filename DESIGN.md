...
`.github/workflows/upstream-varnish-overlay.yml` is an evidence-only workflow, deliberately outside `release.yml`. On Debian 13 amd64 it configures Varnish Software's signed package repository, selects the upstream binary whose version matches the catalogued Varnish release, installs that exact runtime/development pair, and discovers the strict `varnishd-abi-*` capability from native package metadata. It builds the non-upstream `basicauth` VMOD against that installed API, creates a proof package depending on both the exact upstream package version and strict ABI, installs the assembled cohort, starts `varnishd`, and emits `COHORT.json` with provider, package version, ABI, source commit and package digest.
