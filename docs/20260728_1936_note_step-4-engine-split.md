...
Status: Implemented on branch `step4-engine-split`, audit fixes applied, and proven live — see [the live-proof report](20260728_2057_report_step-4-engine-split-live-proof.md) for the run evidence and the package-equivalence tables
...
The live proof and the package-equivalence comparison are recorded separately, in [the live-proof report](20260728_2057_report_step-4-engine-split-live-proof.md): four dispatched runs, all four cases observed as expected, 10/10 Debian packages byte-identical, 18/18 EL9 RPMs semantically equivalent, smoke and behaviour unchanged. This note describes what was built and why; that one records what happened when it ran.
...
- ~~**The live proof.**~~ Done: four dispatched runs, every case observed as expected. See [the live-proof report](20260728_2057_report_step-4-engine-split-live-proof.md).
- ~~**The package-equivalence comparison.**~~ Done: 10/10 Debian packages byte-identical excluding `.buildinfo` and `.changes`, 18/18 EL9 RPMs semantically equivalent, smoke and behaviour unchanged, against the last green `main` run at this branch's base commit. The structural argument above turned out to be right — including for the three Vinyl packages, which a different job built from differently-assembled source and which still came out byte-identical — but, as in Step 3, it was not evidence until it was measured.
- ~~**The engine ABI dependency is the sharpest thing to check.**~~ Checked first, as the audit directed. All three requirements match on both channels, **including `vinyld(vrt) = 23.0`**, the one the rpm identity branch does not pin. The value the metadata comparison could not have constrained came out the same anyway.
- **D5's other half**: VMOD source-archive metadata is still not read back and compared by the target rows. The engine half is done and is the half that had no alternative check. The audit deferred the source half to Phase 3; it is worth closing before Phase 3 multiplies the number of source artifacts.
- **Start latency, now measured rather than guessed.** On the baseline run the engine matrix occupies the first ~10 minutes and the VMOD source job — which needs no engine artifact — waits behind it; the Debian buildroot is built twice per pair. The plan says to optimize only if measurements justify it. These are the measurements; neither looks worth complicating the graph for yet.
