...
Applying the Phase 1 lesson about injections that change what the ledger *should* contain: neither of these does. They change what a row *did*, so they need to reach only the engine job, not every ledger builder. `--inject` is still threaded into `engine-matrix` in `discover-engines` so the marking is computed once, from the same constant the tests use.
