...
Open item 3 of the closing report is closed: main can now produce `complete=true`, and a release-tier draft no longer needs `allow_incomplete_evidence`. Open item 4 (the roughly-20-versus-57 asset-count expectation) remains open; this run reproduced 57, consistent with Proof A's sidecar accounting.
