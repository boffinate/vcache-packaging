...
- **EL10 requires an x86-64-v3 CPU and Rosetta does not emulate that level.** `docker run --platform linux/amd64 almalinux:10` dies instantly with `Fatal glibc error: CPU does not support x86-64-v3`, before any of our script runs. On this arm64 host, EL10 proof runs must be `--platform linux/arm64`; they exercise every script path but produce aarch64 RPMs under an `el10-x86_64` target name. GitHub's amd64 runners are v3-capable, so CI is the only place the real x86_64 artifact exists. Judge a local EL10 run on its steps, never its artifact names.
