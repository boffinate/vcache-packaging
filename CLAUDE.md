...
Local runs write under `work/` (gitignored, disposable: nothing in the tree reads it back, and no document may depend on its contents). CI artifact/layout contracts (result filenames `<row>--<engine>--<target>--<mode>.json`, source-identity directories inside source bundles, engine×target directories inside native-target bundles, and packages under `<workdir>/packages/`) are in DESIGN.md.

## Container facts learned the hard way (2026-08-10 proof runs)

- **Native vs emulated matters a lot on this arm64 host**: cachetag's 52-VTC suite took ~60 min under qemu/amd64 but minutes native. Engine prefix tarballs are arch-specific — never mix an amd64 prefix into a native arm64 run. The target registry selects native x64 and ARM runners, and the scripts reject a platform mismatch.
...
## State as of 2026-09-02 (fleet-watch triage, decisions 24-30)

A page-wide triage of the published matrix (264 red cells, 23 VMODs) found nine root causes. Fixed here, each container-proven on debian-13-arm64 (18 cells green): module discovery follows libtool symlinks (tus, brotli, dispatch had `.libs/libvmod_X.so` as a symlink); the Debian recipe no longer runs `dh_auto_clean` (libvmod-re's distclean deleted its tracked README.rst); `package.install_target` scopes the install phase (zipflow's `make install` built a test program against the engine build tree); `make check` detail walks every `test-suite.log`; the normalizer probes the engine for `cache_vinyld.h` vs `cache_int.h` and runs a same-family `.vsc` pass; trunk engines publish a numeric `pkgconfig_version` in their `.pc` files; every Autotools build appends `-Wno-error`. Catalog: curl re-pinned and opted into normalization, brotli and dispatch opted in, file re-pinned to its Vinylized master with the field flipped (promoted, so the next wave needs a `package_revision` bump), zipflow gains `install_target`.

Vinyl trunk diverges from Varnish trunk in three places that no harness change can bridge (report upstream): `6d36364cc1` dropped the installed `cache_vinyld.h` name (Varnish keeps a `cp` shim; the probe above covers it), `d70e7430be` made `WS_Reservation` a macro so bare-symbol configure probes fail (querystring), `86feb4d23d` moved `struct vmod_data` to `vmod_abi.h` without updating vmodtool's include boilerplate (tus). Still red and upstream-only: Rust VMODs on Vinyl (a `varnishapi.pc` shim was evaluated and would still fail on `BSSL_F_*` TLS symbols Vinyl lacks), oidc's varnish-sys 0.5.5, queryfilter (no Varnish 9 branch), tinykvm on arm64. Not provable on this host: the tinykvm x86 cells (same-family `.vsc` pass, `src/kvm/test-suite.log` detail); the next x86 CI run is their verdict.

Two harness facts learned: the tool shell on this host is zsh, so `set -- $string` does not split (write helper scripts in bash); and the CI job-log download endpoint is `gh api repos/.../actions/runs/<id>/logs` (a zip), not `gh run view --log`, which refuses runs of this size.
...
Settled since: the ABI/cohort Provides question is closed as *no change* (DESIGN.md decision 10), backed by a container proof of apt/dpkg upgrade behaviour. The generated VMOD RPM now arch-qualifies its engine dep (`Requires: vinyl-cache%{?_isa} = <ver>`) to match the engine spec's own `-devel` subpackage.
