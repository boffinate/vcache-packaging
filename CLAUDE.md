...
Proven by real local container runs: vinyl-9.0.1 engine (prefix + debs), dict compat+package green, redis red-on-vinyl/green-on-varnish, varnish-9.0.3 engine, cachetag build + suite (52/53, known flake). In flight at session end: the native-arch verification of `tests: make-check` end-to-end, the `test_failed` negative path, and varnish-modules' cells — check `git log` and `work/verify2/` for how far it got; any fixes it produced may be uncommitted.
