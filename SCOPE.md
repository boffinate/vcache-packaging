...
- Upstream freshness comes from live checks of each selected upstream (`tools/upstream_watch.py`), not from point-in-time survey snapshots. A moved release tag is a loud failure, never an automatic re-pin.
- A **new** upstream release tag is, by maintainer decision of 2026-07-30, an automation trigger: once detection is reliable, the project intends to package new upstream releases unattended, so that a security release lands even while the maintainer is away ([decision and design questions](docs/20260730_1334_note_maintainer-vision-and-automated-release-decision.md)). The automated path travels the same evidence pipeline as a deliberate one — re-pin with recorded digest, full CI evidence, transaction measurement, completeness-gated assembly — and anything red stops the release and surfaces to the maintainer. Until that automation exists and its authority model (in particular, who or what flips a draft to a published release) is recorded here, action on new tags remains deliberate.
