...
- selected independently packaged VMODs:
  ...
  - `libvmod-redis` (`github.com/carlosabalde/libvmod-redis`), added 2026-07-29 by maintainer decision. The selection itself was delegated to the coordinating agent on the same basis as `vmod-dict`, and it was chosen to exercise **one** useful variation rather than to broaden coverage: it is the first selected VMOD that cannot be built against Vinyl Cache unmodified, so it is what the reviewed-source-patch capability is for. The rationale is recorded in [`docs/20260729_1240_note_step-7-wave-1-redis-exception.md`](docs/20260729_1240_note_step-7-wave-1-redis-exception.md). Selected source: tag `9.0-23.1`, commit `b6ca669fc9af3399f3845d9d4930683b4e378aa8`, version `23.1` — the tag names the Varnish series and the VMOD version, and `AC_INIT` is the authority for the latter. Upstream publishes no release archive at all, so the archive is derived deterministically from the tag and its digest pinned. First production lane: Vinyl, engine `vinyl-release` only — `vinyl-trunk-pinned` is excluded for the same root cause as `vmod-dict`, that Vinyl trunk emits no numeric version — on targets `debian-13-amd64` and `el9-x86_64`. Packaging recipes are generated in this repository; upstream provides none. Packaging carries one reviewed, digested patch that renames the Varnish build-system interfaces to their Vinyl equivalents and changes no program source. Behaviour verification: upstream's own 20 VTCs, ported by a two-token rename and run against the installed package through upstream's own suite runner, which launches the Redis servers they address.
...
- Upstream freshness comes from live checks of each selected upstream (`tools/upstream_watch.py`), not from point-in-time survey snapshots. A moved release tag is a loud failure, never an automatic re-pin. The watcher also covers, watch-only, the active VMOD fleet roster (`registry/fleet-watch.json`): fleet observations surface packaging candidates to the maintainer and never gate a run, create a CI row, or build anything.
- A **new** upstream release tag is, by maintainer decision of 2026-07-30, an automation trigger: once detection is reliable, the project intends to carry new upstream releases unattended through the same evidence pipeline as a deliberate release — re-pin with recorded digest, full CI evidence, transaction measurement, completeness-gated assembly — so that a security release is ready even while the maintainer is away ([decision and design questions](docs/20260730_1334_note_maintainer-vision-and-automated-release-decision.md)). Anything red stops that release and surfaces to the maintainer.
- Publication authority is decided (2026-07-30, [publication-authority decision](docs/20260730_1635_note_publication-authority-decision.md)): automation detects upstream movement, verifies pin integrity, and notifies loudly; it publishes nothing. All publication — flipping a GitHub draft to a published release, and promotion into any future managed repository — is a deliberate human action. A release that lands while the maintainer is away may wait, or be driven remotely through the same deliberate dispatches; that trade-off is accepted. The gate applies to releases only: the scheduled, change-gated trunk early-warning lane remains fully automatic, with no human trigger, and a red run notifies.
...
### Responsibility added by the third VMOD (2026-07-29)

Adding `libvmod-redis` adds the same evidence increment again — a third VMOD package family on both targets, with the full set: clean-room build, lint under the same hard gates, installed-package smoke, installed-package behaviour suite, upgrade-transaction matrix, and hardening inspection.

It adds two obligations the first two did not.

**A reviewed patch is a maintenance commitment.** The patch is pinned by digest against one upstream tag. It does not carry forward to the next release for free: every version bump has to re-derive it, review it again, and record the new digest, and an upstream that restructures its build system can make that work rather than a rebase. This is accepted deliberately as the price of the capability, and it is bounded — the patch is build-system only, per VMOD, and visible in the manifest. A blanket substitution pass or a shared shim layer would not be bounded and stays out of scope.

**A behaviour suite with fixture servers is a heavier gate.** The suite installs a Redis server from the distribution and starts up to nine instances per case. That makes the row slower, gives it a dependency on a distribution package outside the selected set, and makes it sensitive to the Redis version each distribution ships. It is still the right gate: load-only verification was ruled explicitly insufficient at Step 5, and upstream's own expectations against the installed object are the strongest available statement that the package works.
