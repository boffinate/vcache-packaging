...
The `experiment/vcache-api-grid` branch runs release and trunk copies of the production matrix with the upstream-neutral Autotools conversion and an exact Vinyl Cache commit. It stores results separately and uploads a self-contained HTML matrix as a workflow artifact; it does not deploy to GitHub Pages. Pushing the branch runs both lanes, so the workflow remains usable without a dispatcher on `main`.
