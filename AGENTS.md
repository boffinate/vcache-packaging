...
- `tools/` — Python 3 standard-library tooling that validates the manifests and generates native package version metadata. Entry point `tools/release_tool.py`; `tools/vmod_recipe.py` renders the generated VMOD recipes, `tools/ci_matrix.py` owns the VMOD catalog and the CI ledger, `tools/upstream_watch.py` answers whether any selected upstream has moved, and `tools/repin_prepare.py` turns one of its re-pin candidates into a reviewable branch without publishing anything.
...
- `docs/` — design notes and session records that are current: active plans, decision records, step closing reports, and notes recording still-live constraints. `docs/archive/` holds superseded plans and historical wave-by-wave/diagnostic notes, retained for ideas and provenance; see `docs/archive/README.md`.
...
`ci_matrix.py selftest` also runs the recipe generator's, the upstream watcher's and the re-pin preparer's tests, so the CI structural-validation gate covers all four.
...
It answers four questions per VMOD — does the pinned tag still peel to the recorded commit (a moved tag is a loud failure, never a re-pin candidate), are there stable tags sorting above the pin (surfaced to the maintainer, never acted on; pre-release-shaped tags are informational only), has a tag name ever been observed at a different commit than first recorded (poisoned: permanent moved-tag treatment, even after it returns), and has a watched trunk branch moved — plus Vinyl trunk HEAD. It also watches the Vinyl release tag itself under the same moved-tag rule, the watched trunk branches cover cachetag, dict, and redis, and it observes the watch-only fleet roster (`registry/fleet-watch.json`) for new stable tags, which surface as informational packaging candidates and never gate anything. It replaces the survey JSON as the freshness signal per the [2026-07-30 maintainer decisions](docs/20260730_0826_note_step-8-maintainer-decisions.md); notification policy (per-tag issues for pinned rows, one rolling fleet digest) is in the [publication-authority decision note](docs/20260730_1635_note_publication-authority-decision.md).

Since 2026-07-30 the same workflow goes one step further for pinned rows and **prepares** the re-pin: `tools/repin_prepare.py` turns a re-pin candidate into a branch `auto-release/<vmod>-<tag>` and a pull request recording the tag, the re-peeled commit and the fetched archive's digest, with `ci.yml` dispatched against the branch for evidence. It still publishes nothing, merges nothing, and writes no evidence file — `apply` refuses to write under `registry/targets/`, `registry/cohorts/` or a transactions expectation at all. Eligibility is read from the registry, not from a list: an engine row, a VMOD with a reviewed patch, a version a cohort manifest cross-checks into recorded evidence, or a derived archive is refused with its reason and stays issue-only. Design and deferrals: [auto-prepared re-pin note](docs/20260730_1812_note_auto-prepared-repin-pr.md).

```sh
python3 tools/repin_prepare.py eligibility --report <watch-report.json>
python3 tools/repin_prepare.py selftest
```

Generated VMOD recipes (host-safe, stdlib only, builds nothing):
